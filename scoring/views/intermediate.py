...
def _make_dynamic_formset_for_initial(initial_len: int):
    ...
    desired = min(DEFAULT_EXTRA, remain)  # cap을 넘지 않게
...
def search(request, client_id=None):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('additems') == 'true':
            ...
            total_after = min(TOTAL_CAP, total_now + 1)
            formset_dictionary_copy['form-TOTAL_FORMS'] = str(total_after)
            ...
            if formset.is_valid():
                ...
                for idx, form in enumerate(formset.forms, start=1):
                    ...
                    form.instance.card = to_roman(cd.get('card', ''))

                    card = normalize_card_to_num(form.instance.card)  # '1'~'10'
...
def export_structural_summary_xlsx(request, client_id):
    ...
    for field_name in fields:
        ws.cell(row=s_row, column=7, value=field_name)  # G열 라벨
        s_row += 1
    ...
    for field_name in cont_fields:
        ws.cell(row=s_row, column=10, value=field_name)  # J열
        s_row += 1

    ws['M4'] = "approach"
    for r, label in enumerate(['I','II','III','IV','V','VI','VII','VIII','IX','X'], start=5):
        ws.cell(row=r, column=13, value=label)  # M열 라벨
    ...
    for field_name in sp_fields:
        ws.cell(row=s_row, column=13, value=field_name)  # M열
        s_row += 1
    ...
    start_row = 6
    for blend in blends:
        ws.cell(row=start_row, column=6, value=blend)  # F열
        start_row += 1

    row = 6
    for field_name in real_field:
        field_value = getattr(structural_summary, field_name)
        ws.cell(row=row, column=8, value=field_value)  # H열
    ...
    for field_name in cont_real_fields:
        field_value = getattr(structural_summary, field_name)
        ws.cell(row=row, column=11, value=field_value)  # K열
    ...
    lv1_row = 18
    for field_name in ['sp_dv','sp_inc','sp_dr','sp_fab','sp_alog','sp_con']:
        ws.cell(row=lv1_row, column=14, value=getattr(structural_summary, field_name))  # N열
        lv1_row += 1
    lv2_row = 18
    for field_name in ['sp_dv2','sp_inc2','sp_dr2','sp_fab2']:
        ws.cell(row=lv2_row, column=15, value=getattr(structural_summary, field_name))  # O열
        lv2_row += 1
    ...
    row = 15
    for field_name in ['xa_per','wda_per','x_minus_per','s_minus','popular','x_plus_per','xu_per']:
    ...
    row = 15
    for field_name in ['Zf','W_D_Dd','W_M','Zd','sp_psv','dev_plus','dev_v']:
    ...
    row = 15
    for field_name in ['ego','fr_rf','sum_V','fdn','an_xy','sp_mor','h_prop']:
    ...
    import pandas as pd
    response_codes_qs = ResponseCode.objects.filter(client=client_id)
    ...
    response_codes_sorted = sorted(response_codes_qs, key=lambda rc: (_card_num(rc), _n(rc)))
    ...
    for col_num, column_title in enumerate(response_code_df.columns, 1):
        c = ws2.cell(row=1, column=col_num, value=column_title)
        c.font = Font(bold=True)

    for row_vals in response_code_df.values.tolist():
        ws2.append(row_vals)
